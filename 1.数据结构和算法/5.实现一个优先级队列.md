...
class PriorityQueue:
    ...
    def push(self, item, priority):
        """ 优先级取priority的复数，当调用heapq.heappop()时，取出优先级高的相当于最小项 """
...
# 讨论
这一小节我们主要关注` heapq `模块的使用。 函数` heapq.heappush() `和` heapq.heappop() `分别在队列` _queue `上插入和删除第一个元素， 并且队列` _queue `保证第一个元素拥有最高优先级（ 1.4 节已经讨论过这个问题）。
